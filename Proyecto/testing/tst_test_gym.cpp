...
TEST_CASE("Agenda Agregacion") {
    ...
    SECTION("Chequeo Archivos ") {
        REQUIRE((fileClient.is_open() && fileClasses.is_open() &&
                 fileAssistances.is_open()));
    }
...
TEST_CASE("check findAssistances") {
    ...
    Asistencia *assistance = findAssistances(assitances, 4, "4");
...
TEST_CASE("lectura archivo asistencias") {
    ifstream fileAssistances("../../asistencias_1700601755", ios::binary);

    REQUIRE(fileAssistances.is_open());
